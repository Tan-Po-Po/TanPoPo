...
export const WindowMatchMediaProvider = ({
  ...
  useEffect(() => {
    ...
    screenMobile.matches &&
      dispatch(
        setWindowMatchMedia({ isMobile: true, isTablet: false, isPc: false })
      );
    screenTablet.matches &&
      dispatch(
        setWindowMatchMedia({ isMobile: false, isTablet: true, isPc: false })
      );
    ...
    screenTablet.onchange = (e) => {
      if (e.matches) {
        dispatch(
          setWindowMatchMedia({ isTablet: true, isMobile: false, isPc: false })
        );
      }
    };
    screenPc.onchange = (e) => {
      if (e.matches) {
        dispatch(
          setWindowMatchMedia({ isPc: true, isMobile: false, isTablet: false })
        );
      }
    };
  }, [dispatch]);
